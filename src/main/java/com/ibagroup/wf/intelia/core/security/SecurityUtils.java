...
public class SecurityUtils {
	...
	/**
	 * @param binding
	 *            - Binding implementation, usually from webharvest config
	 */
	public SecurityUtils(Binding binding) {
		...
		this.aliasDs = dataStoreAccess.executeQuery("UserAliasesPerApplication", "select * from @this where status = '" + CommonConstants.INACTIVE + "';").getSelectResultAsListRows().get().stream()
				.map(row -> {
	...
	/**
	 * @param appName
	 *            - alias name to look up on datastore aliasDs
	 * @return return List of SecureEntryDtoWrapper for security aliases stored in aliasDs for given application alias aliasName
	 */
	...
	/**
	 * @param aliasString
	 *            - alias name in secure data store
